	...
	try {
		...
		const getDatas = () => {
			...
			$('.tdqty').each(function (index, element) {
				...
				let obj = {
					size:idsize, // id product
					sumprod:idsumprod,
					lastwk:idgroup // id group
				};
				results.push(obj);
			});
			return results;
		...
		const getConfigRelation = (datas) => {
			$.ajax({
				...configAjax,
				url: "index.php?route=api/getConfigRelation",
				async: true,
				success: function( results ){  
					// console.log(results);
					let groupcodeRelation = results.map(v => v.group);
					console.log(groupcodeRelation);
					// datas.filter(f => f.size)
				},
		...
		let datas = getDatas(); // get text
		// datas.map(v => {
		// 	console.log(v.size);
		// 	// $('.tdmsg[data-idproduct='+idproduct+']').data('text');
		// })
		getConfigRelation(datas);
...
$(document).ready(function () {
	...
	const pasteLastwk = () => {
		...
		$('.tdqty').each(function (index, element) {
			...
			// 				console.log('association/ajaxCountBarcodeNotuse', idgroup, idproduct, data);
